    ...
    (function() {
      ...
      var newBC = function(initial_size) {
        ...
        return {
          ...
          getBase: function(index2) {
            if (base.array.length - 1 < index2) {
              return -index2 + 1;
            }
            return base.array[index2];
          },
          getCheck: function(index2) {
            if (check.array.length - 1 < index2) {
              return -index2 - 1;
            }
            return check.array[index2];
          },
          setBase: function(index2, base_value) {
            if (base.array.length - 1 < index2) {
              realloc(index2);
            }
            base.array[index2] = base_value;
          },
          setCheck: function(index2, check_value) {
            if (check.array.length - 1 < index2) {
              realloc(index2);
            }
            check.array[index2] = check_value;
          },
          setFirstUnusedNode: function(index2) {
            first_unused_node = index2;
          },
      ...
      DoubleArrayBuilder.prototype.isUnusedNode = function(index2) {
        var bc = this.bc;
        var check = bc.getCheck(index2);
        if (index2 === ROOT_ID) {
          return false;
        }
        if (check < 0) {
      ...
      DoubleArray.prototype.contain = function(key) {
        ...
        for (var i = 0; i < buffer.length; i++) {
          var code = buffer[i];
          child = this.traverse(parent2, code);
          if (child === NOT_FOUND) {
            return false;
          }
      ...
      DoubleArray.prototype.traverse = function(parent2, code) {
        var child = this.bc.getBase(parent2) + code;
        if (this.bc.getCheck(child) === parent2) {
          return child;
        } else {
          return NOT_FOUND;
        }
...
var SurrogateAwareString = class {
  constructor(str) {
    this.str = str;
    this.index_mapping = [];
  ...
  slice(index2) {
    if (this.index_mapping.length <= index2)
      return "";
    const surrogate_aware_index = this.index_mapping[index2];
    return this.str.slice(surrogate_aware_index);
  }
  charAt(index2) {
    if (this.str.length <= index2)
      return "";
    const surrogate_aware_start_index = this.index_mapping[index2];
    const surrogate_aware_end_index = this.index_mapping[index2 + 1];
...
var Tokenizer = class {
  ...
  static splitByPunctuation(input) {
    ...
    while (true) {
      if (tail === "")
        break;
      const index2 = tail.search(PUNCTUATION);
      if (index2 < 0) {
        sentences.push(tail);
        break;
      }
      sentences.push(tail.substring(0, index2 + 1));
...
var ConnectionCosts = class {
  ...
  put(forward_id, backward_id, cost) {
    const index2 = forward_id * this.backward_dimension + backward_id + 2;
    if (this.buffer.length < index2 + 1)
      throw "ConnectionCosts buffer overflow";
    this.buffer[index2] = cost;
  }
...
var ByteBuffer = class {
  ...
  getShort(index2) {
    ...
    if (this.buffer.length < index2 + 2)
      return 0;
    const lower = this.buffer[index2];
    const upper = this.buffer[index2 + 1];
  ...
  getInt(index2) {
    ...
    if (this.buffer.length < index2 + 4)
      return 0;
    const b0 = this.buffer[index2];
    const b1 = this.buffer[index2 + 1];
    const b2 = this.buffer[index2 + 2];
    const b3 = this.buffer[index2 + 3];
...
var DynamicDictionaries = class {
  constructor(trie, token_info_dictionary, connection_costs, unknown_dictionary) {
    this.trie = trie != null ? trie : import_doublearray.default.builder(0).build([
      { k: "", v: 1 }
    ]);
    this.token_info_dictionary = token_info_dictionary != null ? token_info_dictionary : new TokenInfoDictionary_default();
    this.connection_costs = connection_costs != null ? connection_costs : new ConnectionCosts_default(0, 0);
    this.unknown_dictionary = unknown_dictionary != null ? unknown_dictionary : new UnknownDictionary_default();
  }
...
var DictionaryLoader = class {
  ...
  load(load_callback) {
    ...
    async_default.parallel([
      function(callback) {
        async_default.map(["base.dat.gz", "check.dat.gz"], (filename, _callback) => {
          return loadArrayBuffer(import_path.default.join(dic_path, filename), (err, buffer) => {
            if (err)
      ...
      function(callback) {
        async_default.map(["tid.dat.gz", "tid_pos.dat.gz", "tid_map.dat.gz"], (filename, _callback) => {
          loadArrayBuffer(import_path.default.join(dic_path, filename), (err, buffer) => {
            if (err)
              return _callback(err);
            _callback(null, buffer);
          });
      ...
      function(callback) {
        loadArrayBuffer(import_path.default.join(dic_path, "cc.dat.gz"), (err, buffer) => {
          if (err)
            return callback(err);
      ...
      function(callback) {
        async_default.map(["unk.dat.gz", "unk_pos.dat.gz", "unk_map.dat.gz", "unk_char.dat.gz", "unk_compat.dat.gz", "unk_invoke.dat.gz"], (filename, _callback) => {
          loadArrayBuffer(import_path.default.join(dic_path, filename), (err, buffer) => {
            if (err)
              return _callback(err);
            _callback(null, buffer);
  ...
  loadArrayBuffer(file, callback) {
    import_fs.default.readFile(file, (err, buffer) => {
      if (err)
        return callback(err);
      import_zlib.default.gunzip(buffer, (err2, decompressed) => {
        if (err2)
